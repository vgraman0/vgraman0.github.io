...
![ccc text](image-4.png)


HTTP/0.9 was introduced by Tim-Berners Lee as part of the World Wide Web. It only supported `GET` requests and did not have not have headers or status codes (`200`, `404`, `500`, etc.). These were introduced in HTTP/1.0 along with `POST` and `HEAD`. 
...
- If the first result gets stuck, all subsequent requests are also blocked, eben if they are ready to be sent.
