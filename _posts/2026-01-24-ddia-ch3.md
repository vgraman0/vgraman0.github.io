...
**Write amplification** 
Overwriting blocks on SSDs will eventually cause the performance to degrade. 
...
**Storage**
- LSM-trees are not page-oriented and periodically rewrite SSTables to remove fragentation, resulting in lower storage overhead.
...
Instead, store all values from each column together in separate files.
...
when the number of distinct values is very large, the bitmaps will be sparse. In that case, we can use a **run-length encoding** which makes the representation of the binary value string more compact.
