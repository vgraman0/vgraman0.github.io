...
- Changing a datatype is possible if the reader can resolve the writer's type into its own. This is governed by **type promotion**; e.g. widening from `int` to `long`.
...
- Adding a branch to a **union type** is also backwards compatible but not forwards compatible. This is required if you want to allow a field to be `null` since that is not an acceptable default in Avro.
